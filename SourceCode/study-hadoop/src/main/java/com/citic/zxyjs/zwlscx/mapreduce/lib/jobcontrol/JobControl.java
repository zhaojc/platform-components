package com.citic.zxyjs.zwlscx.mapreduce.lib.jobcontrol;
...
import com.citic.zxyjs.zwlscx.mapreduce.lib.jobcontrol.ControlledJob.State;
 ...
 * their states. This class provides APIs for the client app to add a job to the
 * group and to get the jobs in the group in different states. When a job is
 * added, an ID unique to the group is assigned to the job. This class has a
 * thread that submits jobs when they become ready, monitors the states of the
 * running jobs, and updates the states of jobs based on the state changes of
 * their depending jobs states. The class provides APIs for suspending/resuming
...
public class JobControl implements Runnable {
    ...
    synchronized private static List<ControlledJob> toList(LinkedList<ControlledJob> jobs) {
