...
public class MetaDataHelper {
	...
	if (source instanceof File) {
	    return getFieldByFileName(source.getName());
	}
	return getFieldByTableName(source.getName());
    }

    //TODO parent fields - rule fields
    public static List<Field> getFieldByName(JoinTask joinTask) {
	List<Field> fields = new ArrayList<Field>();
	List<Source> sources = joinTask.getOutput().getParentSource();
