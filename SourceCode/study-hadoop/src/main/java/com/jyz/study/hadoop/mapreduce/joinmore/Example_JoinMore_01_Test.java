...
public class Example_JoinMore_01_Test {
	...
	job.setJarByClass(Example_JoinMore_01_Test.class);
	// 设置Map相关内容
	job.setMapperClass(Example_JoinMore_01_Mapper.class);
	// 设置Map的输出
	job.setMapOutputKeyClass(TextPair.class);
	job.setMapOutputValueClass(Text.class);
	// 设置partition
	job.setPartitionerClass(Example_JoinMore_01_Partitioner.class);
	// key比较函数
	job.setSortComparatorClass(Example_JoinMore_01_KeyComparator.class); 
	// 在分区之后按照指定的条件分组
	job.setGroupingComparatorClass(Example_JoinMore_01_Comparator.class);

	job.setReducerClass(Example_JoinMore_01_Reduce.class);
	// 设置reduce的输出
