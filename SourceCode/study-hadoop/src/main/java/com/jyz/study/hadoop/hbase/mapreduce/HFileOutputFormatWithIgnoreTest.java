...
/**
 * 使用HBase作为数据流向 读取hdfs文件存放到hbase中 -t testtable -i test-data.txt -c data:json -o
 * tmpoutput test-data.txt的数据被加载到testtable data:json中， rowkey为md5(line)
 * value为line
 * 
 * @author JoyoungZhang@gmail.com
 */
// vv ImportFromFile
public class HFileOutputFormatWithIgnoreTest {
    // ^^ ImportFromFile
    private static final Log LOG = LogFactory.getLog(HFileOutputFormatWithIgnoreTest.class);

    // vv ImportFromFile
    public static final String NAME = "ImportFromFileWithBulkLoad"; // co

    // ImportFromFile-1-Name
    // Define a
    // job name
    // for later
    // use.
     ...
     */
    // vv ImportFromFile
    static class ImportMapper extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> { // co
	// ImportFromFile-2-Mapper
	// Define
	// the
	// mapper
	// class,
	...
	// vv ImportFromFile
	@Override
	public void map(LongWritable offset, Text line, Context context) // co
		// ImportFromFile-3-Map
		// The
		// map()
		// function
		// transforms
		...
		put.add(family, qualifier, Bytes.toBytes(lineString)); // co
		// ImportFromFile-5-Put
		// Store
		// the
		// original
		// data
     ...
     */
    // vv ImportFromFile
    private static CommandLine parseArgs(String[] args) throws ParseException { // co
	// ImportFromFile-6-ParseArgs
	// Parse
	// the
	// command
	// line
	...
	o.setRequired(true);
	options.addOption(o);
	o = new Option("o", "tmpoutput", true, "the tmp directory or file to write to");
	o.setArgName("path-in-HDFS");
	o.setRequired(true);
	options.addOption(o);
	...
	String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs(); // co
	// ImportFromFile-7-Args
	// Give
	// the
	// command
	// line
	...
	Job job = new Job(conf, "Import from file with bulkload " + input + " into table " + table); // co
	// ImportFromFile-8-JobDef
	// Define
	// the
	// job
	// with
	...
	// required
	// classes.
	job.setJarByClass(HFileOutputFormatWithIgnoreTest.class);
	...
	// job.setNumReduceTasks(2); // co ImportFromFile-9-MapOnly This is a
	// map only job, therefore tell the framework to bypass the reduce step.
