...
/**
 * 使用HBase作为数据源与数据流向 但自定义处理 只使用TableMapReduceUtil.initTableMapperJob 因为map阶段就已经
 * infoTable.put(infoPut) MapReduce作业解析每行数据为若干列 -i testtable -c data:json -o
 * infotable -l linktable 将testtable data:json的数据划分为 infotable
 * data:otherColumn和linktable data:link两个表
 * 
 * @author zhaoyong.zhang create time 2014-3-11
 */
public class ParseJsonMulti {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
	    infoTable = new HTable(context.getConfiguration(), context.getConfiguration().get("conf.infotable")); // co
	    // ParseJsonMulti-1-Setup
	    // Create
	    // and
	    // configure
	    // both
	    ...
	    linkTable = new HTable(context.getConfiguration(), context.getConfiguration().get("conf.linktable"));
	...
	protected void cleanup(Context context) throws IOException, InterruptedException {
	    ...
	    linkTable.flushCommits(); // co ParseJsonMulti-2-Cleanup Flush all
	    // pending commits when the task is
	    // complete.
	}
	...
	public void map(ImmutableBytesWritable row, Result columns, Context context) throws IOException {
		...
		for (KeyValue kv : columns.list()) {
			...
			if ("link".equals(key)) {
			    linkPut.add(columnFamily, Bytes.toBytes(key.toString()), Bytes.toBytes(val.toString()));
			} else {
			    infoPut.add(columnFamily, Bytes.toBytes(key.toString()), Bytes.toBytes(val.toString()));
			}
		    }
		}
		infoTable.put(infoPut); // co ParseJsonMulti-3-TwoTbls Save
		// parsed values into two separate
		// tables.
	...
	// vv ParseJsonMulti
	conf.set("conf.infotable", cmd.getOptionValue("o")); // co
	// ParseJsonMulti-4-Conf
	// Store table
	// names in
	// configuration
	// for later use in
	...
	TableMapReduceUtil
		.initTableMapperJob(input, scan, ParseMapper.class, ImmutableBytesWritable.class, Put.class, job, false);
	job.setOutputFormatClass(NullOutputFormat.class); // co
	// ParseJsonMulti-5-Null
	// Set the output
	// format to be
	// ignored by the
	// framework.
