...
/**
 * 使用HBase作为数据源 TableMapReduceUtil.initTableMapperJob -t testtable -c data:json
 * -o output1 分析habse数据 author1 : num1 author2 : num2 author3 : num3 。。。
 * 
...
public class AnalyzeData {
     ...
     */
    // vv AnalyzeData
    static class AnalyzeMapper extends TableMapper<Text, IntWritable> { // co
	// AnalyzeData-1-Mapper
	// Extend
	// the
	// supplied
	// TableMapper
	...
	public void map(ImmutableBytesWritable row, Result columns, Context context) throws IOException {
		...
		for (KeyValue kv : columns.list()) {
		    ...
		    String author = (String) json.get("author"); // co
		    // AnalyzeData-2-Parse
		    // Parse the
		    // JSON data,
		    // extract the
     ...
     */
    // vv AnalyzeData
    static class AnalyzeReducer extends Reducer<Text, IntWritable, Text, IntWritable> { // co
	// AnalyzeData-3-Reducer
	// Extend
	// a
	// Hadoop
	// Reducer
	...
	// vv AnalyzeData
	Scan scan = new Scan(); // co AnalyzeData-5-Scan Create and configure a
	// Scan instance.
	if (column != null) {
	...
	// Path("file:/D:/Maven/repo/com/googlecode/json-simple/json-simple/1.1/json-simple-1.1.jar"));
	// DistributedCache.addFileToClassPath(new
	// Path("/libs/hbase-0.92.1-cdh4.0.0-security.jar"),
	// job.getConfiguration());
	job.setJarByClass(AnalyzeData.class);
	TableMapReduceUtil.initTableMapperJob(table, scan, AnalyzeMapper.class, Text.class, IntWritable.class, job, false); // co
	// AnalyzeData-6-Util
	// Set
	// up
	// the
	...
	job.setOutputKeyClass(Text.class); // co AnalyzeData-7-Output Configure
	// the reduce phase using the normal
	// Hadoop syntax.
