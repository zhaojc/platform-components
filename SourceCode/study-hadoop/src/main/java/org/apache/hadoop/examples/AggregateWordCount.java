 ...
 * 和ValueAggregatorJob已经实现各种数据类型的求和最大值，最小值的算法。类型如下： UniqValueCount LongValueSum
 * DoubleValueSum ValueHistogram LongValueMax LongValueMin StringValueMax
 * StringValueMin 具体请看相关的源代码。 这个job的执行必须用-jarlibs执行，不然会报configured错误。 执行命令如下：
 * hadoop jar hadoop-example.jar -libjars hadoop-example.jar shakepoems.text
 * out_aggregate_his 3 textinputformat
 * 
 * @author zhaoyong.zhang create time 2014-3-12
 */
