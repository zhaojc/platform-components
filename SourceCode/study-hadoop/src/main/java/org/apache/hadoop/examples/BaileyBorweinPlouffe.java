...
public class BaileyBorweinPlouffe extends Configured implements Tool {
    ...
    private static final String NAME = "com.jyz.study.hadoop.hbase.mapreduce." + BaileyBorweinPlouffe.class.getSimpleName();
	...
	protected void map(LongWritable offset, IntWritable length, final Context context) throws IOException,
	...
	@Override
	protected void reduce(LongWritable offset, Iterable<BytesWritable> values, Context context) throws IOException,
		InterruptedException {
	    // read map outputs
    ...
    private static <T> void print(PrintWriter out, Iterator<T> iterator, String prefix, String format, int elementsPerGroup,
	    int groupsPerLine) {
	...
	if (startDigit <= 0) {
	    ...
	    throw new UnsupportedOperationException("nDigits - 1 + startDigit = " + (nDigits - 1L + startDigit)
		    + " > IMPLEMENTATION_LIMIT + BBP_HEX_DIGITS," + ", where IMPLEMENTATION_LIMIT=" + IMPLEMENTATION_LIMIT
